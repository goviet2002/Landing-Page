...
const SkillsGraph = ({ skills }: SkillsGraphProps) => {
  ...
  useEffect(() => {
    ...
    const drawSkillsData = (
      ...
      for (let i = 0; i < sides; i++) {
        const angle = (Math.PI * 2 * i) / sides - Math.PI / 2
        const value = (skills[i].level / 100) * progress
      ...
      const firstValue = (skills[0].level / 100) * progress
      ctx.lineTo(
      ...
      for (let i = 0; i < sides; i++) {
        ...
        if (progress > 0.9) {
          ...
          ctx.textAlign = "center"
          ctx.textBaseline = "middle"
          ctx.fillText(`${Math.round(skills[i].level)}%`, valueX, valueY)
        }
      }
    }
