  ...
  return (
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="bg-[#1e293b] border-cyan-500/30 text-white max-w-[1100px] w-full max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-cyan-400">Courses</DialogTitle>
        </DialogHeader>
        <div className="mt-4">
            ...
            {courses.map((course, idx) => (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <div className="font-semibold text-cyan-300 text-base">{course.name}</div>
                  {course.semester && (
                ...
                {course.grade && (
                  <div className="text-xs text-cyan-400 font-medium mb-1">Grade: {course.grade}</div>
                )}
                {course.description && (
